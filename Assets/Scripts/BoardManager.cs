...
public class BoardManager : MonoBehaviour
{
    ...
    private Vector3 whiteCapturedPosition = new Vector3(10, 0, 0);
    private Vector3 blackCapturedPosition = new Vector3(-2, 0, 8);
    ...
    private void MoveChessman(int x, int y)
    {
        if (allowedMoves[x, y])
        {
            ...
            StartCoroutine(SmoothMove(selectedChessman, GetTileCenter(x, y)));
    ...
    public void EndGame()
    {
        ...
        activeChessman.Clear(); // Clear the list of active chessmen
        isWhiteTurn = true;
        BoardHighlights.Instance.HideHighlights();
    ...
    {
        SpawnAllChessmans(); // Respawn all chessmen
        EnPassantMove = new int[2] { -1, -1 }; // Reset en passant move
        // Any other variables or game states you need to reset
    }

    private IEnumerator SmoothMove(Chessman piece, Vector3 target)
    {
        float elapsedTime = 0;
        ...
        {
            piece.transform.position = Vector3.Lerp(startingPos, target, (elapsedTime / duration));
    ...
    private IEnumerator CapturePiece(Chessman piece)
    {
        float elapsedTime = 0;
        float duration = 0.5f; // Duration of the animation in seconds
        Vector3 startingPos = piece.transform.position;
        ...
        if (piece.isWhite)
        {
            int row = capturedWhitePieces.Count / 4;
            int col = capturedWhitePieces.Count % 4;
            target = whiteCapturedPosition + new Vector3(col * TILE_SIZE, 0, row * TILE_SIZE);
            capturedWhitePieces.Add(piece.gameObject);
        }
        else
        {
            int row = capturedBlackPieces.Count / 4;
            int col = capturedBlackPieces.Count % 4;
            target = blackCapturedPosition + new Vector3(-col * TILE_SIZE, 0, -row * TILE_SIZE);
            capturedBlackPieces.Add(piece.gameObject);
        }
